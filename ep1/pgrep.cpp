...
/* Função que adiciona '/' ao final do caminho, caso este não possua. Se o caminho não tiver o '/' ao final, a função get_files() não executa corretamente */
string check_path(char *path)
{
    string p = path;
    if (p.back() != '/')
    {
...
*/
void get_files(const string &path, vector<string> &files, const bool show_hidden = true)
{
    DIR *dir;
    struct dirent *epdf;
...
int main(int argc, char *argv[])
{
    ...
    get_files(check_path(PATH), files, false);
    ...
    }                           //O Número de Threads é atuaizado, para que não sejam criadas threads ociosas.
